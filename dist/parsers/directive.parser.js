...
var DirectiveParser = (function (_super) {
    ...
    DirectiveParser.prototype._parseTemplate = function (template, path) {
        ...
        $(template)
            ...
            var context = $element.attr('translate-context');
            var comment = $element.attr('translate-comment');
            var reference = path;
            if (attr) {
                collection = collection.add(attr, null, { context: context, comment: comment, reference: reference });
            }
            else {
                $element
                    .contents()
                    ...
                    .forEach(function (text) { return collection = collection.add(text, null, { context: context, comment: comment, reference: reference }); });
            }
        });
        return collection;
    };
